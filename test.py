...
def criar_arquivo():
    try:
        with open('arquivo_vendas.bin', 'wb') as arquivo:
        ...
        with open('arquivo_vendas.bin', 'rb') as arquivo:
            while True:
                ...
                if not registro:
                    break
                registro_data = struct.unpack(registro_format, registro)
                print(f"Código do vendedor: {registro_data[0]}, Valor da venda: {registro_data[1]}, Mês e ano: {registro_data[2].decode()}")
    except Exception as e:
...
def consultar_maior_venda():
    try:
        ...
        if maior_venda:
            print(f"Vendedor com maior valor de venda: Código: {maior_venda[0]}, Valor: {maior_venda[1]}, Mês e ano: {maior_venda[2].decode()}")
        else:
